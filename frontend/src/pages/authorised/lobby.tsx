...
import HistoryTab from '../../components/lobby/HistoryTab';
import LobbyHeader from '../../components/lobby/LobbyHeader';
import GameRoomBrowser from '../../components/lobby/GameRoomBrowser';
...
// frontend/src/pages/authorised/lobby.tsx
// Add this function to handle local game creation properly
const handleCreateLocalGame = (gameType: "pong" | "keyclash") => {
  // Generate a unique ID for the local game
  const gameId = `local-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  
  // Navigate directly to the game without using sockets
  navigate(`/${gameType}/local/${gameId}`, { 
    state: { 
      name: user.name,
      isLocal: true 
    } 
...
// Update your create functions to handle local games properly:
const createLocalPong = () => {
  // Generate a unique ID for local game
  const localGameId = `local-pong-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  joinGame(localGameId, "pong", "local");
};

const createLocalKeyClash = () => {
  // Generate a unique ID for local game
  const localGameId = `local-keyclash-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  joinGame(localGameId, "keyclash", "local");
};

// For remote games, keep the socket emission:
const createRemotePong = () => {
  socketRef.current?.emit("create_game", "pong", "remote");
};
...
// In your LobbyPage component, update the joinGame function:
const joinGame = (gameId: string, game: "pong" | "keyclash", mode: "local" | "remote") => {
  if (mode === "local") {
  ...
  // Show Game Browser when Quick Match or Championship is clicked
  if (showGameBrowser) {
    return (
      <GameRoomBrowser 
        type={browserType}
