...
import { useAuth } from './contexts/AuthContext';
import { Layout, LoadingSpinner, ErrorBoundary } from './components/general';
import { GameProtectedRoute } from './components/general/GameProtectedRoute';
  ...
  return (
    <ErrorBoundary fallback={
      ...
      <Suspense fallback={
        ...
        <Routes>
          {/* 🎮 GAME ROUTES - PROTECTED BUT NO NAVBAR (Full Screen Games) */}
          <Route path="/" element={<Menu />} />
          <Route path="/play" element={
            <GameProtectedRoute><PlayPage /></GameProtectedRoute>
          } />
          <Route path="/:game/:mode/:gameId" element={
            <GameProtectedRoute><PlayPage /></GameProtectedRoute>
          } />

          {/* 🔐 AUTH & LOBBY ROUTES - WITH NAVBAR */}
          <Route element={<Layout />}>
            {/* Public auth routes */}
            <Route path="/login" element={
              <PublicRoute><LoginPage /></PublicRoute>
