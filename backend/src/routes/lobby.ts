...
// Update user's last activity (call this every 30 seconds from frontend)
  fastify.post('/lobby/heartbeat', {
  preHandler: authenticate
  }, async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = (request as any).user;
 ...
 // Get online users (anyone active in last 2 minutes)
 fastify.get('/lobby/online-users', async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    fastify.log.error(error);
    return reply.status(500).send({ error: 'Failed to fetch online users' });
  }
 });

 // Set user offline when they logout
 fastify.post('/lobby/set-offline', {
  preHandler: authenticate
 }, async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = (request as any).user;
    ...
    fastify.log.error(error);
    return reply.status(500).send({ error: 'Failed to set offline' });
  }
 });
